...
tags:
    - Computer Science
---
## https://www.bilibili.com/video/BV1hA411v7qX/?spm_id_from=333.999.0.0&vd_source=7798c62f92ce545f56fd00d4daf55e26
- 可视化客户端：GitKraken、SourceTree、SmartGit、TortoiseGit
- 代码托管：github、gitlab、bitbucket、gitee（可以直接clone github的项目）
- amend：对已提交的文件&信息进行修改
    - 最好对还未push到远程的提交使用，因为已经提交到远程的提交，别人可能已经拉取了，这样会造成跟别人的版本不一致
- branch
- stash：将还不打算提交的代码暂存，代码会恢复到之前的状态
    - pop：恢复暂存的代码
- rebase(变基)：将整个分支摘取下来，嫁接在另一个分支上，这样提交历史不会分叉
    - pull的时候也可以选择rebase
    - merge的优点：原先的提交历史会被完全保留
- fast-forward(快进)：master指针指向最新的提交
- checkout：切换分支，切换版本
- 撤销：git的所有操作都记录在reflog里面
    - 不要尝试修改远程服务器的提交历史
    - git revert：生成一个与想要撤销的提交完全相反的新提交
