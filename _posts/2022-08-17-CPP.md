...
---
## 引用 vs 智能指针
https://www.reddit.com/r/cpp_questions/comments/msdnes/smart_pointers_on_heap_vs_stack/
https://stackoverflow.com/questions/42910711/unique-ptr-heap-and-stack-allocation
https://stackoverflow.com/questions/12119637/smart-pointers-vs-reference
...
- https://www.bilibili.com/video/BV1Sr4y1H7qE/?spm_id_from=333.880.my_history.page.click&vd_source=7798c62f92ce545f56fd00d4daf55e26
- 栈内存：stack
...
- unique_ptr：独享指针
    ...
    - unique_ptr销毁或释放的时候，其绑定的资源也会自动释放，不再需要手动释放
        - 即便抛出异常，资源最后也会被释放
    - unique_ptr的使用跟裸指针类似
    - 不能有两个unique_ptr指向同一个地址，不支持普通拷贝，但是可以转移控制权

    ```C
    unique_ptr<int> up1 = make_unique<int>(100);
    unique_ptr<int> up2(up1.release());  // up1释放资源的控制权，并且返回裸指针
    // 或者
    unique_ptr<int> up3 = move(up1);
    ```
    
    - 在函数间传递unique_ptr: 由于禁止复制，因此不能pass by value
    ...
    ```C
    unique_ptr<int> return_ptr(int value)
    {
    ...
    unique_ptr<int> up = return_ptr(123);  // 编译器自动调用了move constructor 
    ```
...
## 协程
https://www.bilibili.com/video/BV1RV4y1L7ar/?spm_id_from=333.999.0.0
    - up：等疾风
...
## 常用网站
https://zhuanlan.zhihu.com/p/466675250
CppCon
SD West
...
- 陈硕的blog
https://www.cnblogs.com/solstice/category/287661.html
...
## 书
- https://www.zhihu.com/question/430318186
...
- https://www.zhihu.com/question/493402906/answer/2436485586