...
`Blob`是`Caffe`的基础数据结构，提供了统一的数据处理接口，同时隐藏了CPU/GPU异构编程的细节。`Bolb`底层是按照行优先存储(`C-contiguous`)的多维数组。`Blob`采用`NCHW`的顺序存储，也就是同一行/通道/batch的数据在内存中连续存储。因此，对于索引为` (n, k, h, w) `的数据，其实际索引为` ((n * K + k) * H + h) * W + w`（K,H,W分别为通道数、高度、宽度）。
...
`Layer`是神经网络的基本单元，包括卷积、pooling、点乘、`relu`、`sigmoid`等。
...
`Net`即神经网络，将`Layer`连接形成计算图，计算图是一个有向无环图(`DAG`)，`Caffe`中神经网络的定义非常直观，下图是一个简单的逻辑回归分类器

![1678254092491](C:\Users\宋伟清\AppData\Roaming\Typora\typora-user-images\1678254092491.png)
...
神经网络的计算过程由`Forward`和`Backword`组成：

![1678258163072](C:\Users\宋伟清\AppData\Roaming\Typora\typora-user-images\1678258163072.png)

`Foward`阶段按照计算图中`Layer`的顺序，对输入数据进行前向推导



![1678259235032](C:\Users\宋伟清\AppData\Roaming\Typora\typora-user-images\1678259235032.png)

`Backward`阶段通过自动微分(`automatic differentiation`)将梯度进行反向传播，通过链式法则计算每一层的梯度。

![1678259346180](C:\Users\宋伟清\AppData\Roaming\Typora\typora-user-images\1678259346180.png)
...
`Data Layer`负责加载数据成`Blob`，数据变换，输出数据。
...
## [Caffe如何计算卷积](https://caffe.berkeleyvision.org/tutorial/convolution.html)
