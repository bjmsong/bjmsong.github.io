...
tags:
    - 图算法
---
>本文将介绍图表示学习的几种算法，包括DeepWalk、LINE、GNN等。
...
<ul> 
<li markdown="1"> 
最近几年，图神经网络比较火。下面这张图是近些年GNN论文发表数量的情况，我们可以看到从2016年开始，GNN论文发表数量呈指数上升趋势。
...
<ul> 
<li markdown="1"> 
GNN结合了信息传播机制和神经网络，信息会在网络上传播，改变相邻结点的状态，直到达到平衡。
先定义一些符号，结点代表实体或者概念，边代表它们之间的关系。点跟边的属性用l表示，每个结点的状态用x表示。每个顶点的状态，参数函数fw称为局部变换函数，描述了结点和其邻域的依赖性。gw称为局部输出函数，刻画了输出值的生成过程。
...
<ul> 
<li markdown="1"> 
上面那个是原图，下面那个是与之对应的图传播机制的展示，把每个结点表示成fw和gw的计算过程，fw和gw都可以用前向神经网络来表示。
