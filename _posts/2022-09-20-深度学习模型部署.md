...
- https://www.zhihu.com/people/zhang-xiao-yu-45-67-74/posts

## On-Device Machine Learning from data to application
https://www.youtube.com/watch?v=a_3Bp8qPueE&list=PLLuMmzMTgVK5oDb94Lq5L19wwD9aRNF5a&index=3&ab_channel=HuaHua
...
1. 模型部署场景
    - 中心服务器云端部署:模型部署在云端服务器，用户通过网页访问或者API接口调用等形式向云端服务器发出请求，云端收到请求后处理并返回结果
    - 边缘部署:主要用于嵌入式设备，主要通过将模型打包封装到SDK，集成到嵌入式设备，数据的处理和模型推理都在终端设备上执行。
2. 模型部署方式
    - Service 部署: 主要用于中心服务器云端部署，一般直接以训练的引擎库作为推理服务模式
    - SDK 部署: 主要用于嵌入式端部署场景，以 C++ 等语言实现一套高效的前后处理和推理引擎库（高效推理模式下的 Operation/Layer/Module的实现），用于提供高性能推理能力。此种方式一般需要考虑模型转换（动态图静态化）、模型联合编译等进行深度优化
3. 部署的核心优化指标 
    - 成本：主要是芯片
    - 功耗：在数据中心服务场景，对于功耗的约束要求相对较低；在边缘终端设备场景，硬件的功耗会影响边缘设备的电池使用时长。因此，对于功耗要求相对较高
    - 性价比：云端相对更加关注是多路的吞吐量优化需求，而终端场景则更关注单路的延时需要。
4. 部署流程（以 SDK 部署）
    - 模型转换
    - 模型量化压缩
    - 模型打包封装 SDK
...
- ONNX(Open Neural Network Exchange)
    ...
    - https://space.bilibili.com/278317189/channel/series
...
- https://pytorch.org/tutorials/beginner/Intro_to_TorchScript_tutorial.html
    ...
    - TorchScript provides tools to capture the definition of model, even in light of the flexible and dynamic nature of PyTorch
        ...
        - Mixing Scripting and Tracing
            - 在trace模型中调用script，适合模型中只有一小部分需要用到控制流的情况
...
- https://zhuanlan.zhihu.com/p/354058294
...
https://www.zhihu.com/question/428800593
