...
![../../../_images/caffe_code_overview.png](https://hqli.github.io/_images/caffe_code_overview.png)
...
- `cpu_ptr_`：指向host内存首地址的指针，可以指向任何类型的数据
- `gpu_ptr_`：指向device内存首地址的指针，可以指向任何类型的数据
- `size_`：内存占多少字节
- `head_`：标识当前数据在什么设备上，有四个枚举值：
  - `UNINITIALIZED`
  - `HEAD_AT_CPU`
  - `HEAD_AT_GPU`
  - `SYNCED`
...
- 如果`head_=UNINITIALIZED`:  分配`host`动态内存空间，大小为`size_`，并全部初始化为0
- 如果`head_=HEAD_AT_GPU`:  分配`host`动态内存空间（如果还没有分配的话），调用`cudaMemcpy`把`device`数据拷贝到`host`
...
`Blob`是`Caffe`的基础数据结构，提供了统一的数据处理接口，同时隐藏了CPU/GPU异构编程的细节。`Blob`采用`NCHW`的顺序存储，也就是同一行/通道/batch的数据在内存中连续存储。因此，对于索引为` (n, k, h, w) `的数据，其实际索引为` ((n * K + k) * H + h) * W + w`（K,H,W分别为通道数、高度、宽度）。
...
- data_
  - `Blob`的数据
  - 是一个`shared_ptr`，指向`SyncedMemory`对象，当`data\_`生命周期结束，会自动调用``SyncedMemory``的析构函数释放内存
- diff_
  - `Blob`的梯度
  - 是一个`shared_ptr`，指向`SyncedMemory`对象
- shape_data: `Blob`的形状，是一个`shared_ptr`
- shape_: `Blob`的形状，跟`shape_data`一个是老版本一个是新版本
- count_: `Blob`的元素个数，也就是batch数\*通道数\*高度\*宽度
- capacity\_:  分配的内存空间可以容纳的元素个数，必须大于等于count_
...
- 如果`shape_data_`原来的内存不够，会重新分配内存，并释放原来的内存。
- 给`count_`，`shape_`，`shapre_data_`赋值。
...
```c++
/// @brief Deprecated legacy shape accessor width: use shape(0) instead.
...
`caffe`使用`protocol buffer`来进行数据的序列化和反序列化。`caffe`定义了一系列的`protobuf message`，其中， `BlobProto` 来保存和读取 `Blob` 数据，`BlobProto` 的定义如下：
...
- 重置了`BlobProto` 消息类型`shape`,`data`,`diff`字段，并把`Blob`对象的值添加到相应字段
...
- `cblas_sasum`,`cblas_ddot`,`cblas_sscal`...
- `cublasSasum`,`cublasSdot`,`cublasDscal`...
